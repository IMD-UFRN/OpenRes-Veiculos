...
class DatePickerInput < SimpleForm::Inputs::StringInput
  def input
    ...
    input_html_options[:data].merge!({ dateFormat: picker_pettern, language: I18n.locale.to_s,
    ...
    template.content_tag :div, class: 'input-append date datepicker' do
      ...
      input += template.content_tag :span, class: 'add-on' do
        template.content_tag :i, '', class: 'fa fa-calendar-o', data: { 'time-icon' => 'icon-time', 'date-icon' => 'icon-calendar' }
      end
      input
    end
  end
